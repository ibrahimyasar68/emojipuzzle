...
| K-23 | **Sürüm notları** her sürümde iki dilde yazılır ve `docs/store-listing.md` içinde durur: Play dil başına 500 karakterde keser, çok dilli alanda metinler BCP-47 etiketleriyle (`<tr-TR>`, `<en-US>`) sarılır. Notlar `pubspec.yaml`'daki sürümle aynı numarayı taşır; test bunu denetler. *(20 Eylül, kullanıcı istedi.)* | §33 |
| K-22 | **Mağaza açıklamasında gelişimsel beceriler.** Türkçe ve İngilizce uzun açıklamalara, oyunun çocukta gerçekten çalıştırdığı şeyleri anlatan bir bölüm eklendi (parça–bütün, görsel ayırt etme, el–göz koordinasyonu, sabır, renk seçimi, sınıflandırma). Ölçülebilir öğrenme iddiası yok. Google Play'in **Öğretmen Onaylı** değerlendirmesi bu kritere bakar. *(20 Eylül, kullanıcı istedi.)* | §33 |
| K-21 | **Giriş ekranı yenilendi.** Oyna düğmesi artık gülen bir suratın **dört parçalı bitmiş yapbozu**dur; sağ alt köşesinde küçük kırmızı bir oynat işareti vardır. Yüz `assets/images/ui/smile.png` — kodla çizildi (K-16'nın betiği), puzzle havuzuna girmez. Bilgi düğmesi bir **yetişkin kapısı** oldu: iki saniye basılı tutmak gerekir, tutarken halka dolar (§2'nin "uzun basma yok" kuralı çocuğun oynamak için yapmak zorunda olduğu hareketler içindir; bu onun tersidir). Hakkında ekranında oyunun kısa tarifi, **IY Labs** ve **ibrahimyasar68@hotmail.com** bulunur. Mağaza sayfasına ikinci dil olarak **İngilizce** metinler eklendi. *(20 Eylül, kullanıcı istedi.)* | §2, §29, §33 |
| K-20 | **Play Store yayını.** Paket kimliği `com.iylabs.emojipuzzle` olarak değişti (yayından sonra değişmez; eski kimlikle kurulu ilerleme sıfırlanır — kullanıcı kabul etti), sürüm `1.0.0+1`, yayın imzası `android/key.properties`'ten okunur, ekran yönü kilidi kaldırıldı (§40'ın yatay yerleşimleri artık cihazda da görünür). Mağaza görselleri `docs/store/`, adımlar `docs/play-store-release.md`. *(20 Eylül, kullanıcı istedi.)* | §35, §40 |
...
| **28** | Sürüm notları, iki dilde ve etiketli (K-23) | İki dilde not var, ikisi de 500 karakterin altında ve sürüm numarası pubspec ile aynı |
| **27** | Mağaza metnine gelişimsel beceriler, iki dilde (K-22) | İki açıklamada da bölüm var, ikisi de 4000 karakterin altında |
| **26** | Giriş ekranı: yapboz oyna düğmesi, yetişkin kapısı, İngilizce mağaza metni (K-21) | Düğme dört parça + kırmızı işaret, ⓘ iki saniye ister, Hakkında'da IY Labs ve iletişim, listing iki dilde |
| **25** | Play Store hazırlığı (K-20) | Paket kimliği ve sürüm doğru, yayın imzası key.properties'ten, mağaza görselleri ölçüsünde, izin yok |
